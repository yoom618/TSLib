...
class Model(nn.Module):
    ...
    def encoder(self, x):
        B, T, N = x.size()
        # padding : moved from classification since it also occurs in other tasks (e.g. anomaly detection)
        x = torch.cat([x, torch.zeros((x.shape[0], self.seq_len-x.shape[1], x.shape[2])).to(x.device)], dim=1)
