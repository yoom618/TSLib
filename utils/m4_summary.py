...
def group_values(values, groups, group_name):
    return np.array([v[~np.isnan(v)] for v in values[groups == group_name]], dtype=np.ndarray)
...
class M4Summary:
    ...
    def evaluate(self):
        ...
        naive2_forecasts = np.array([v[~np.isnan(v)] for v in naive2_forecasts], dtype=np.ndarray)
